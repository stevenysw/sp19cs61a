...
def prune_leaves(t, vals):
    ...
    1
      ...
      6
    """
    if label(t) in vals and is_leaf(t):
        return None
    else:
        new = []
        for b in branches(t):
            if prune_leaves(b, vals):
                new = new + [prune_leaves(b, vals)]
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        l, r = left(m), right(m)
        left_torque, right_torque = total_weight(end(l)) * length(l), total_weight(end(r)) * length(r)
        return balanced(end(l)) and balanced(end(r)) and left_torque == right_torque
